	...
	<div class="px-4 pt-16 pb-12 md:px-8">
		<div class="mx-auto max-w-lg">
			<div class="mb-6 flex items-center gap-4">
				...
				<h1 class="text-2xl font-bold text-dark">Edit Account</h1>
			</div>
			...
			<form onsubmit={handleSubmit} class="flex flex-col gap-6">
					...
					<div class="flex flex-wrap gap-2 md:gap-3">
						{#each ACCENT_COLORS as c (c)}
							<button
								type="button"
								onclick={() => (color = c)}
								class="size-6 rounded-full transition-all md:size-8"
								style="background-color: {c}; {color === c
									? `box-shadow: 0 0 0 2px white, 0 0 0 4px ${c};`
									: ''}"
				...
				<!-- Balance -->
				<label class="flex flex-col gap-2">
					<span class="text-sm font-medium text-dark">Balance</span>
					<input
						type="number"
						min="0"
						step="0.01"
